...
> **Security:** Never hardcode secrets. Prefer setting `JIRA_URL`, `JIRA_EMAIL`, and `JIRA_API_TOKEN` in your system enprojectnment (or a secrets manager). Only use the MCP `env` block for local, uncommitted config files.

**To get a Jira API token:**
1. Go to <https://id.atlassian.com/manage-profile/security/api-tokens>
2. Click **Create API token**
3. Copy the token — store it in your enprojectnment, never in source code
...
Store these in your shell enprojectnment, secrets manager, or an untracked local env file. Do not commit them to the repo.
...
- **Always use** enprojectnment variables or a secrets manager
