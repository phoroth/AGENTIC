...
```python
import os, pytest
os.enprojectn["QT_ACCESSIBILITY"] = "1"  # Required for Qt 5.x UIA support
...
def app(request):
    if not APP_PATH:
        pytest.exit("APP_PATH enprojectnment variable is not set", returncode=1)
...
```python
import os
APP_PATH          = os.enprojectn.get("APP_PATH", "")           # set via env — no default path
MAIN_WINDOW_TITLE = os.enprojectn.get("APP_TITLE", "")
LAUNCH_TIMEOUT    = int(os.enprojectn.get("LAUNCH_TIMEOUT", "15"))
ACTION_TIMEOUT    = int(os.enprojectn.get("ACTION_TIMEOUT", "10"))
...
```python
import os, json, time
TRACE_ENABLED      = os.enprojectn.get("E2E_TRACE") == "1"
TRACE_INCLUDE_TEXT = os.enprojectn.get("E2E_TRACE_INCLUDE_TEXT") == "1"
...
# Skip in CI only
@pytest.mark.skipif(os.enprojectn.get("CI") == "true", reason="Flaky in CI #43")
...
def app(request, tmp_path):
    ...
    sandbox_env = os.enprojectn.copy()
    ...
    if not APP_TITLE:
        pytest.exit("APP_TITLE enprojectnment variable is not set", returncode=1)
...
jobs:
  e2e:
    runs-on: windows-latest   # real GUI enprojectnment, no Xvfb needed
    steps:
...
Qt 5.x accessibility is disabled by default in some builds (especially 5.7–5.14). Set the enprojectnment variable **before** launching. Qt 6.x enables accessibility by default — skip this step for Qt 6.

```python
# conftest.py — add at module top
import os
os.enprojectn["QT_ACCESSIBILITY"] = "1"
```
