...
- Run phases sequentially from enprojectnment checks through deployment readiness so each layer builds on the last.
- Enprojectnment and Composer checks gate everything else; stop immediately if they fail.
...
- Confirm `APP_DEBUG=false` for production enprojectnments
...
- Confirm `storage/` and `bootstrap/cache/` are writable in the target enprojectnment
...
Only run this on non-production enprojectnments where processing a test job is safe.
