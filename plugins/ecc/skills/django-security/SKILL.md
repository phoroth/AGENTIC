...
ALLOWED_HOSTS = os.enprojectn.get('ALLOWED_HOSTS', '').split(',')
...
# Secret key (must be set via enprojectnment variable)
SECRET_KEY = os.enprojectn.get('DJANGO_SECRET_KEY')
if not SECRET_KEY:
    raise ImproperlyConfigured('DJANGO_SECRET_KEY enprojectnment variable is required')
...
For enprojectnments where installing libmagic is difficult (e.g., minimal containers),
...
```python
# Use python-decouple or django-enprojectn
import enprojectn
...
| Strong secrets | Use enprojectnment variables for SECRET_KEY |
