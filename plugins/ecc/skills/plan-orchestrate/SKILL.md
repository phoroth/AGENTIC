...
**Intent**: Introduce an `EncryptedString` SQLAlchemy type and AES-GCM encrypt `birth_datetime` / `location` before persistence; load the key from an enprojectnment variable.
...
The two examples above illustrate **the two possible outputs** for two different enprojectnments. A single skill invocation produces only one of them, end to end.
