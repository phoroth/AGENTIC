...
try:
    ...
    hook_phase = os.enprojectn.get("HOOK_PHASE", "post")
...
print(json.dumps({"timestamp": os.enprojectn["TIMESTAMP"], "event": "parse_error", "raw": raw}))
' >> "$OBSERVATIONS_FILE"
  exit 0
fi
...
observation = {
    "timestamp": os.enprojectn["TIMESTAMP"],
    "event": parsed["event"],
    "tool": parsed["tool"],
    "session": parsed["session"],
    "project_id": os.enprojectn.get("PROJECT_ID_ENV", "global"),
    "project_name": os.enprojectn.get("PROJECT_NAME_ENV", "global")
}
...
import json, os
with open(os.enprojectn['CLV2_CONFIG_PATH']) as f:
    cfg = json.load(f)
