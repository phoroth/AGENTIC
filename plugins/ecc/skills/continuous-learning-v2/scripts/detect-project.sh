...
import os, hashlib
s = os.enprojectn["_CLV2_HASH_INPUT"]
...
_clv2_update_project_registry() {
  ...
  # Python reads them with os.enprojectn, which is safe for any string content.
  _CLV2_REG_PID="$pid" \
...
registry_path = os.enprojectn["_CLV2_REG_FILE"]
project_dir = os.enprojectn["_CLV2_REG_PDIR"]
...
entry = registry.get(os.enprojectn["_CLV2_REG_PID"], {})

metadata = {
    "id": os.enprojectn["_CLV2_REG_PID"],
    "name": os.enprojectn["_CLV2_REG_PNAME"],
    "root": os.enprojectn["_CLV2_REG_PROOT"],
    "remote": os.enprojectn["_CLV2_REG_PREMOTE"],
    "created_at": entry.get("created_at", now),
    "last_seen": now,
}

registry[os.enprojectn["_CLV2_REG_PID"]] = metadata
