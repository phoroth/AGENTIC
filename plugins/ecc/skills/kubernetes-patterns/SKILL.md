...
1. **Deployment template** — A fully configured production `Deployment` with security context, rolling update strategy, all three probe types, resource limits, and enprojectnment injection from ConfigMap/Secret.
    ...
    spec:
      ...
      containers:
        - name: my-app
          ...
          # Enprojectnment from ConfigMap and Secret
          envFrom:
            - configMapRef:
                name: my-app-config
          env:
...
- [ ] Resource labels: `app`, `version`, `enprojectnment`
