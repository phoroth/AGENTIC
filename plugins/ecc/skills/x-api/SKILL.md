...
bearer = os.enprojectn["X_BEARER_TOKEN"]
...
```bash
# Enprojectnment setup — source before use
...
oauth = OAuth1Session(
    os.enprojectn["X_CONSUMER_KEY"],
    client_secret=os.enprojectn["X_CONSUMER_SECRET"],
    resource_owner_key=os.enprojectn["X_ACCESS_TOKEN"],
    resource_owner_secret=os.enprojectn["X_ACCESS_TOKEN_SECRET"],
)
```
...
- **Never hardcode tokens.** Use enprojectnment variables or `.env` files.
