...
Use an email and API key for production scripts. Store API keys in enprojectnment
...
def esearch(query: str, retmax: int = 20) -> list[str]:
    params = {
        ...
        "email": os.enprojectn.get("NCBI_EMAIL", ""),
    }
    api_key = os.enprojectn.get("NCBI_API_KEY")
    if api_key:
...
- Are API keys loaded from the enprojectnment?
