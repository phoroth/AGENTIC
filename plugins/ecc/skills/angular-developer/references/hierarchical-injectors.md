...
1. **`EnprojectnmentInjector` Hierarchy**: Configured via `@Injectable({ providedIn: 'root' })` or `ApplicationConfig.providers` during bootstrap. These are global singletons.
...
2. If not found, it searches the **`EnprojectnmentInjector`** tree, starting from the closest enprojectnment injector up to the root.
