...
If you need to run a function within an injection context (often needed for dynamic component creation or testing), use `runInInjectionContext`. This requires access to an existing injector (like `EnprojectnmentInjector` or `Injector`).

```ts
import {Injectable, inject, EnprojectnmentInjector, runInInjectionContext} from '@angular/core';

@Injectable({providedIn: 'root'})
export class MyService {
  private injector = inject(EnprojectnmentInjector);
