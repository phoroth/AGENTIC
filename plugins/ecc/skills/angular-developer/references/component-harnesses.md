...
The `TestbedHarnessEnprojectnment` is the entry point for using harnesses in unit tests.

### Example: Testing with a `MatButtonHarness`

```ts
import {TestbedHarnessEnprojectnment} from '@angular/cdk/testing/testbed';
...
describe('MyButtonContainerComponent', () => {
  ...
  beforeEach(async () => {
    ...
    loader = TestbedHarnessEnprojectnment.loader(fixture);
  });
...
1. **`HarnessLoader`**: An object used to find and create harness instances. Get a loader for your component's fixture using `TestbedHarnessEnprojectnment.loader(fixture)`.
