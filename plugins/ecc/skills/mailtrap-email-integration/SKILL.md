...
Patterns for adding transactional email sending to an application using Mailtrap's Email API and Sandbox, covering authentication, enprojectnment separation, and common delivery pitfalls.
...
**Sandbox vs. Production separation.** Mailtrap provides a Sandbox API that captures emails without delivering them, used for dev/staging so test emails never reach real inboxes. Production sending uses a separate, verified-domain endpoint. Never point a dev enprojectnment at the production sending endpoint.
...
```typescript
// Same call, routed to Sandbox in non-production enprojectnments
...
| --- | --- | --- |
| Using the production sending endpoint in dev/test | Real test emails reach real inboxes, risking spam complaints and leaked test data | Route non-production enprojectnments to the Sandbox endpoint |
| Hardcoding API tokens in source | Credential leak risk if committed to version control | Load tokens from enprojectnment variables / secrets manager |
...
- Keep sandbox and production tokens in separate enprojectnment variables, never share one token across enprojectnments
