...
- `config/deploy.yml` is the source of truth for servers, registry, and enprojectnment config
- `.kamal/secrets` references secrets from the host enprojectnment; the file is committed, the secrets are not
...
For most secrets, prefer Rails encrypted credentials (`bin/rails credentials:edit -e production`) over enprojectnment variables. ENV vars are appropriate for infrastructure config that varies per host; credentials are appropriate for application secrets that travel with the codebase.
