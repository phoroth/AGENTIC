...
It's clear how project config, hooks, MCP settings, and enprojectnment variables are part of the execution surface now.
...
That separation matters. One agent can parse a document in a restricted enprojectnment. Another agent, with stronger approvals, can act only on the cleaned summary. Same workflow; much safer.
...
Anthropic has hardened Claude Code and published concrete security guidance around trust, permissions, MCP, memory, hooks, and isolated enprojectnments.
