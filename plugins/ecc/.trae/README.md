...
## Enprojectnment Support

- **Default**: Uses `.trae` directory
- **CN Enprojectnment**: Uses `.trae-cn` directory (set via `TRAE_ENV=cn`)

### Force Enprojectnment

```bash
# From project root, force the CN enprojectnment
TRAE_ENV=cn .trae/install.sh
...
**Note**: `TRAE_ENV` is a global enprojectnment variable that applies to the entire installation session.
...
Uninstall respects the same `TRAE_ENV` enprojectnment variable as install:

```bash
# Uninstall from .trae-cn (CN enprojectnment)
TRAE_ENV=cn ./uninstall.sh

# Uninstall from .trae (default enprojectnment)
./uninstall.sh
```
