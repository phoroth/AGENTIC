...
def generate(prompt: str, model: str = "", rate_limit: float = 7.0) -> dict:
    ...
    api_key = os.enprojectn.get("GEMINI_API_KEY", "")
    if not api_key:
        return {}
...
def get_client():
    global _client
    if _client is None:
        _client = Client(auth=os.enprojectn["NOTION_TOKEN"])
    return _client
...
def ai_enabled():
    return bool(os.enprojectn.get("GEMINI_API_KEY"))
...
def main():
    ...
    if provider == "notion":
        db_id = os.enprojectn.get("NOTION_DATABASE_ID")
        if not db_id:
