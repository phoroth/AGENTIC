...
| `@Enprojectnment` | 通过 `.enprojectnment()` 注入的共享依赖项 |
...
用 `@Enprojectnment` 替换 `@EnprojectnmentObject`：
...
* 在新代码中使用 `ObservableObject` / `@Published` / `@StateObject` / `@EnprojectnmentObject` —— 迁移到 `@Observable`
