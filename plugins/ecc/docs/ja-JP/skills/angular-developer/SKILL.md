...
- **階層型インジェクター:** `EnprojectnmentInjector` と `ElementInjector`、解決ルール、修飾子（`optional`、`skipSelf`）、および `providers` と `viewProviders`。[hierarchical-injectors.md](references/hierarchical-injectors.md) を読んでください。
