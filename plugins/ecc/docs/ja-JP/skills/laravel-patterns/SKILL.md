...
- Centralize config in `config/*` and keep enprojectnments explicit.
...
### Configuration and Enprojectnments

- Keep secrets in `.env` and config in `config/*.php`
- Use per-enprojectnment config overrides and `config:cache` in production
