...
| `@Enprojectnment` | `.enprojectnment()` で注入された共有依存関係 |
...
`@EnprojectnmentObject` の代わりに `@Enprojectnment` を使用する：

```swift
// Inject
ContentView()
    .enprojectnment(authManager)

// Consume
struct ProfileView: View {
    @Enprojectnment(AuthManager.self) private var auth
...
* 新しいコードで `ObservableObject` / `@Published` / `@StateObject` / `@EnprojectnmentObject` を使用する——`@Observable` に移行する
