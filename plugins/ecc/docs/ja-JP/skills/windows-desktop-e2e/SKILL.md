...
```python
import os, pytest
os.enprojectn["QT_ACCESSIBILITY"] = "1"  # Qt 5.x UIAサポートに必要
...
```python
import os
APP_PATH          = os.enprojectn.get("APP_PATH", "")           # 環境変数で設定 — デフォルトパスなし
...
# CIのみでスキップ
@pytest.mark.skipif(os.enprojectn.get("CI") == "true", reason="CIで不安定 #43")
