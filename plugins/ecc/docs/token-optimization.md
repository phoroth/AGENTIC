...
```powershell
[Enprojectnment]::SetEnprojectnmentVariable('ECC_CONTEXT_MONITOR_COST_WARNINGS', 'off', 'User')
```
...
The `configure-ecc` install wizard could offer to set these enprojectnment variables during setup, with explanations of the cost tradeoffs. This would help new users optimize from day one rather than discovering these settings after hitting limits.
...
# Enprojectnment variables (add to ~/.claude/settings.json "env" block)
MAX_THINKING_TOKENS=10000
CLAUDE_CODE_SUBAGENT_MODEL=haiku
