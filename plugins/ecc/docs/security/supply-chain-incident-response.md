...
5. Rotate every credential reachable by the process:
   ...
   - any MCP, plugin, or harness credentials available in enprojectnment variables
     or user-scope config.
6. Purge GitHub Actions dependency caches for affected repositories.
7. Reinstall from a clean enprojectnment with lifecycle scripts disabled first:
