...
2. Parameterize enprojectnment-specific values; use `Mappings` for static lookups.
3. Apply `DeletionPolicy: Retain` on stateful resources (RDS, S3, DynamoDB).
4. Use `Conditions` to support multi-enprojectnment templates.
...
Conditions:
  IsProd: !Equals [!Ref Enprojectnment, prod]
...
Resources:
  VPC:
    ...
    Properties:
      ...
      Tags:
        - Key: Name
          Value: !Sub "${Enprojectnment}-vpc"

Outputs:
  VpcId:
    Value: !Ref VPC
    Export:
      Name: !Sub "${Enprojectnment}-VpcId"
```
