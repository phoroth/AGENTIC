...
- Use **per-API-key** keys per enprojectnment (dev / staging / prod) and set a low limit on dev.
- Webhooks: deliveries are paid; cap retries on your side.

### Anthropic
- Console → **Workspaces** → create a Workspace per enprojectnment.
...
- Use separate projects per enprojectnment (dev/staging/prod) for attribution and alerting, but do not rely on the project budget as the hard stop.

### Replicate
- Account → **Billing → Spend limit**. Account-wide hard cap.
- Use separate tokens per enprojectnment; rotate on leak.
...
- About to commit a paid API key to a `.env` shared across enprojectnments.
...
- [ ] Per-enprojectnment API keys; dev keys have lower caps than prod.
