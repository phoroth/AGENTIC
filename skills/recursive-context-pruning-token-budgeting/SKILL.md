...
Before executing Step 3, scan for missing critical variables (e.g., specific file names or enprojectnment types). If the prompt is too ambiguous, bypass the atomic output and generate exactly one concise question to resolve the blocker.
...
```text
User: "Update the Firebase config to use enprojectnment variables."
```
...
```text
"Specify enprojectnment: production or staging?"
```
...
- Never prune safety headers, enprojectnment-specific security constraints, or system-level instructions during the compression stage.
