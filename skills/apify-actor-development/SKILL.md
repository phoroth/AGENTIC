...
If it is not logged in, check if the `APIFY_TOKEN` enprojectnment variable is defined (if not, ask the user to generate one on https://console.apify.com/settings/integrations and then define `APIFY_TOKEN` with it).

Then authenticate using one of these methods:

```bash
# Option 1 (preferred): The CLI automatically reads APIFY_TOKEN from the enprojectnment.
...
> Prefer enprojectnment variables or interactive login instead.
...
- **Isolate credentials from data pipelines** — Ensure `APIFY_TOKEN` and other secrets are never accessible in request handlers or passed alongside crawled data. Use the Apify SDK's built-in credential management rather than passing tokens through enprojectnment variables in data-processing code.
...
**✓ Do:**
- Use `apify run` to test actors locally (configures Apify enprojectnment and storage)
...
- Hard code values that should be in input schema or enprojectnment variables
...
**IMPORTANT:** Always use `apify run` to test actors locally. Do not use `npm run start`, `npm start`, `yarn start`, or other package manager commands - these will not properly configure the Apify enprojectnment and storage.
