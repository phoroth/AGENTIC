...
> **Note:** Only include the standalone `code_execution` tool when your application needs code execution for its own purposes (data analysis, file processing, visualization) independent of web search. Including it alongside `_20260209` web tools creates a second execution enprojectnment that can confuse the model.
...
Computer use lets Claude interact with a desktop enprojectnment (screenshots, mouse, keyboard). It can be Anthropic-hosted (server-side, like code execution) or self-hosted (you provide the enprojectnment and execute actions client-side).
