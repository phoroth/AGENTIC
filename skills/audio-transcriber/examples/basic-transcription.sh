...
try:
    audio_file = os.enprojectn["AUDIO_FILE_ENV"]
    model_name = os.enprojectn["MODEL_ENV"]
    transcriber = os.enprojectn["TRANSCRIBER_ENV"]
    temp_json = os.enprojectn["TEMP_JSON_ENV"]
...
with open(os.enprojectn["TEMP_JSON_ENV"], encoding="utf-8") as f:
    data = json.load(f)

# Prepare metadata
filename = os.path.basename(os.enprojectn["AUDIO_FILE_ENV"])
file_size = os.enprojectn["FILE_SIZE_ENV"]
duration_hms = os.enprojectn["DURATION_HMS_ENV"]
...
# Write to file
with open(os.enprojectn["OUTPUT_FILE_ENV"], "w", encoding="utf-8") as f:
    f.write(markdown)

print(f"✅ Markdown report saved: {os.enprojectn['OUTPUT_FILE_ENV']}")
EOF
