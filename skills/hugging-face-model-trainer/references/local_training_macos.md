...
@dataclass
class Cfg:
    model_id: str = os.enprojectn.get("MODEL_ID", "Qwen/Qwen2.5-0.5B-Instruct")
    dataset_id: str = os.enprojectn.get("DATASET_ID", "HuggingFaceH4/ultrachat_200k")
    dataset_split: str = os.enprojectn.get("DATASET_SPLIT", "train_sft[:500]")
    data_files: Optional[str] = os.enprojectn.get("DATA_FILES", None)
    text_field: str = os.enprojectn.get("TEXT_FIELD", "")
    messages_field: str = os.enprojectn.get("MESSAGES_FIELD", "messages")
    out_dir: str = os.enprojectn.get("OUT_DIR", "outputs/local-lora")
    max_seq_length: int = int(os.enprojectn.get("MAX_SEQ_LENGTH", "512"))
    max_steps: int = int(os.enprojectn.get("MAX_STEPS", "-1"))
...
BASE = os.enprojectn.get("MODEL_ID", "Qwen/Qwen2.5-0.5B-Instruct")
ADAPTER = os.enprojectn.get("ADAPTER_DIR", "outputs/local-lora")
device = "mps" if torch.backends.mps.is_available() else "cpu"
...
prompt = os.enprojectn.get("PROMPT", "Explain gradient accumulation in 3 bullet points.")
