...
client = SearchClient(
    endpoint=os.enprojectn["AZURE_SEARCH_ENDPOINT"],
    index_name=os.enprojectn["AZURE_SEARCH_INDEX_NAME"],
    credential=AzureKeyCredential(os.enprojectn["AZURE_SEARCH_API_KEY"])
)
```
...
## Enprojectnment Variables for Additional Patterns
...
1. **Use enprojectnment variables** for endpoints, keys, and deployment names
