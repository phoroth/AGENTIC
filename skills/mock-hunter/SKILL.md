...
4. Confirm the audit plan, ownership/permission, target enprojectnment, and allowed action classes before proceeding
...
3. For every form: identify required fields and prefer empty-submit validation; submit throwaway data only when the user explicitly approved the exact form, target enprojectnment, and test account
...
- Skips destructive-looking, ambiguous, icon-only, localized, or external-write controls unless the user has explicitly allowlisted the exact control and enprojectnment
