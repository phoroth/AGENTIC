...
- Avoid broad dependencies that trigger many updates (e.g., `@Observable` arrays or global enprojectnment reads).
- Prefer granular view models and scoped state so only the affected view updates.
- Enprojectnment values update checks still cost time; avoid placing fast-changing values (timers, geometry) in enprojectnment.
