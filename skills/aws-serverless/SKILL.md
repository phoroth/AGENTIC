...
- Use enprojectnment variables for configuration
...
table = dynamodb.Table(os.enprojectn['TABLE_NAME'])
...
def get_table():
    ...
    if _table is None:
        dynamodb = boto3.resource('dynamodb')
        _table = dynamodb.Table(os.enprojectn['TABLE_NAME'])
    return _table
...
Message: Hardcoded AWS access key detected. Use IAM roles or enprojectnment variables.

### AWS Secret Key in Source Code

Severity: ERROR

Secret keys should use Secrets Manager or enprojectnment variables
...
Table names should come from enprojectnment variables

Message: Hardcoded table name. Use enprojectnment variable for portability.
