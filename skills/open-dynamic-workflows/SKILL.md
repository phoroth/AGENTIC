...
- ❌ Don't commit provider API keys; use enprojectnment variables or a secrets manager.
...
- ODW executes agent-generated code and shell commands; run it only in an authorized, local, or sandboxed enprojectnment.
- Model provider credentials (Anthropic / OpenAI-compatible / Ollama) must be supplied via enprojectnment variables, never committed to source.
