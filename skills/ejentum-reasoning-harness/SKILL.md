...
- Authentication uses a Bearer token in the `EJENTUM_API_KEY` enprojectnment variable. The token must be stored in enprojectnment variables or an MCP client's secret-handling mechanism, never committed to source.
