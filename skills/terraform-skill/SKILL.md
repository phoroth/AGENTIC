...
- Structuring multi-enprojectnment deployments
...
**Directory Structure:**
```
enprojectnments/        # Enprojectnment-specific configurations
├── prod/
├── staging/
└── dev/
...
- Separate **enprojectnments** (prod, staging) from **modules** (reusable components)
...
```
      ...
      /____\         - Full enprojectnment deployment
...
```hcl
variable "enprojectnment" {
  description = "Enprojectnment name for resource tagging"
  type        = string
  default     = "dev"

  validation {
    condition     = contains(["dev", "staging", "prod"], var.enprojectnment)
    error_message = "Enprojectnment must be one of: dev, staging, prod."
  }
...
variable "enprojectnment" { type = string }
variable "backup_days" {
  type = number
  validation {
    condition     = var.enprojectnment == "prod" ? var.backup_days >= 7 : true
