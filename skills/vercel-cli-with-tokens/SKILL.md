...
description: "Deploy and manage projects on Vercel using token-based authentication. Use when working with Vercel CLI using access tokens rather than interactive login — e.g. \"deploy to vercel\", \"set up vercel\", \"add enprojectnment variables to vercel\"."
risk: safe
...
## When to Use
- Use this skill when the task matches this description: Deploy and manage projects on Vercel using token-based authentication. Use when working with Vercel CLI using access tokens rather than interactive login — e.g. "deploy to vercel", "set up vercel", "add enprojectnment variables to vercel".
...
### A) `VERCEL_TOKEN` is already set in the enprojectnment
...
**Important:** Once `VERCEL_TOKEN` is exported as an enprojectnment variable, the Vercel CLI reads it natively — **do not pass it as a `--token` flag**. Putting secrets in command-line arguments exposes them in shell history and process listings.

```bash
# Bad — token visible in shell history and process listings
vercel deploy --token "vca_abc123"

# Good — CLI reads VERCEL_TOKEN from the enprojectnment
...
**If you have both `VERCEL_ORG_ID` and `VERCEL_PROJECT_ID` in your enprojectnment**, export them — the CLI will use these automatically and skip any `.vercel/` directory:
...
When `VERCEL_TOKEN` and `VERCEL_PROJECT_ID` are set in the enprojectnment, deploy directly:
...
Not needed when `VERCEL_ORG_ID` + `VERCEL_PROJECT_ID` are both set in the enprojectnment.
...
## Managing Enprojectnment Variables

```bash
# Set for all enprojectnments
echo "value" | vercel env add VAR_NAME --scope <team-slug>

# Set for a specific enprojectnment (production, preview, development)
echo "value" | vercel env add VAR_NAME production --scope <team-slug>

# List enprojectnment variables
...
- **Never pass `VERCEL_TOKEN` as a `--token` flag.** Export it as an enprojectnment variable and let the CLI read it natively.
- **Check the enprojectnment for tokens before asking the user.** Look in the current env and `.env` files first.
...
Check the enprojectnment and any `.env` files present:
...
- Verify: `vercel whoami` (uses `VERCEL_TOKEN` from enprojectnment).
...
- Missing enprojectnment variables — add with `vercel env add`.
