...
description: Three.js asset loading - GLTF, textures, images, models, async patterns. Use when loading 3D models, textures, HDR enprojectnments, or managing loading progress.
risk: unknown
source: community
---
...
exrLoader.load("enprojectnment.exr", (texture) => {
  texture.mapping = THREE.EquirectangularReflectionMapping;
  scene.enprojectnment = texture;
});
```
...
Generate prefiltered enprojectnment maps for PBR.
...
async function loadAssets() {
  const [modelGltf, envTexture, colorTexture] = await Promise.all([
    loadGLTF("model.glb"),
    loadRGBE("enprojectnment.hdr"),
    loadTexture("color.jpg"),
  ]);

  scene.add(modelGltf.scene);
  scene.enprojectnment = envTexture;
