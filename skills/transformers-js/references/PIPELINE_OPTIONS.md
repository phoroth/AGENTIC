...
- [Configuration Reference](./CONFIGURATION.md) - Enprojectnment configuration with `env` object
...
This document covers all available options for the `pipeline()` function. For enprojectnment-level configuration (remote hosts, global cache settings, WASM paths), see the [Configuration Reference](./CONFIGURATION.md).
