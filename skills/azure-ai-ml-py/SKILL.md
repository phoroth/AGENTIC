...
ml_client = MLClient(
    ...
    subscription_id=os.enprojectn["AZURE_SUBSCRIPTION_ID"],
    resource_group_name=os.enprojectn["AZURE_RESOURCE_GROUP"],
    workspace_name=os.enprojectn["AZURE_ML_WORKSPACE_NAME"]
)
```
...
job = command(
    ...
    enprojectnment="AzureML-sklearn-1.0-ubuntu20.04-py38-cpu@latest",
    compute="cpu-cluster",
...
### Create Custom Enprojectnment

```python
from azure.ai.ml.entities import Enprojectnment

env = Enprojectnment(
    name="my-env",
    version="1",
    image="mcr.microsoft.com/azureml/openmpi4.1.0-ubuntu20.04",
    conda_file="./enprojectnment.yml"
)

ml_client.enprojectnments.create_or_update(env)
```
...
| `enprojectnments` | create_or_update, get, list |
| `datastores` | create_or_update, get, list, get_default |
| `components` | create_or_update, get, list |

## Best Practices

1. **Use versioning** for data, models, and enprojectnments
2. **Configure idle scale-down** to reduce compute costs
3. **Use enprojectnments** for reproducible training
