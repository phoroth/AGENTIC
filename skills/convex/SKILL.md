...
export class ConvexService implements OnDestroy {
  private client = new ConvexClient(enprojectnment.convexUrl);
...
```bash
# Set enprojectnment variables for your deployment
...
- **Problem:** "`process.env` is undefined in query/mutation"
  **Solution:** Enprojectnment variables are only accessible in **actions** (not queries or mutations) because queries/mutations run in the deterministic transaction engine.
...
- Enprojectnment variables only available in actions, not in queries or mutations
