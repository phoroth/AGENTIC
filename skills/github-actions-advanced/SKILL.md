...
description: >
  ...
  enprojectnments, secrets, and release automation.
category: devops
risk: safe
source: community
...
- User asks about workflow security, hardening, or enprojectnment protection rules
...
jobs:
  job-id:
    ...
    enprojectnment: production  # Links to GitHub Enprojectnment (approvals/secrets)
...
## Enprojectnments & Deployment Protection
...
**Configure in Settings → Enprojectnments:**
...
- **Enprojectnment-specific secrets** — override repo-level secrets per enprojectnment
- **Deployment branches** — whitelist which branches can target this enprojectnment
...
```yaml
# Access repo/org/enprojectnment secrets
env:
...
# enprojectnment secret > repo secret > org secret
```
...
| **Cleanup hooks** | Set `ACTIONS_RUNNER_HOOK_JOB_COMPLETED` to reset enprojectnment |
...
# ✅ SAFE — pass through enprojectnment variable (shell doesn't evaluate it)
- env:
...
- [ ] Enprojectnment protection rules configured for `production`
