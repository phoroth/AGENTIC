...
description: "High-level technical SEO and site architecture auditor. Invoke to scan local or live enprojectnments for indexing, crawl budget, and structural errors."
category: growth
risk: safe
source: self
source_type: self
...
- **SSR & Hydration**: Verify if Googlebot is seeing the same content as users in JavaScript-heavy enprojectnments (Next.js/Nuxt). Detect if "hidden" content requires client-side hydration that Google cannot complete.
