...
- Use when auditing an existing Agent Harness for missing ECL lifecycle docs, change templates, lint checks, enprojectnment contracts, or CI integration.
...
2. **Analysis** — architecture, harness state, enprojectnment, and project identity.
...
4. **Creation/Update** — docs, status handoff, linters, ECL/change scripts, enprojectnment config, and CI.
...
|-------|----------|----------|
| **Core harness** | Yes | AGENTS.md, docs/ECL.md, docs/STATUS.md, docs, ECL changes, lightweight auto-evolve, linters, enprojectnment contract, CI |
  ...
  "options": [
    {
      ...
      "description": "Project-first AGENTS.md, ECL changes, STATUS handoff, auto-evolve threshold checks, linters, enprojectnment contract, and strict CI"
    },
    {
...
**Goal**: Deeply understand codebase architecture, harness state, and enprojectnment requirements.

### 2.1 Execution Mode

Use subagents only when the user authorized delegation and the enprojectnment supports it. Otherwise, execute the same responsibilities inline.
...
- Enprojectnment analysis: follow `references/enprojectnment-detection-guide.md`; output `harness/.analysis/enprojectnment.json`.
...
or enprojectnment config. Adapter guidance overrides generic templates for language-specific details.
...
cat harness/.analysis/enprojectnment.json
```
...
- [ ] harness/config/enprojectnment.json
...
- Config and scripts: follow `agents/creator-config.md`; create/update enprojectnment contract, harness scripts, changes directories/templates, lightweight evolution state, harness-change, harness-evolve, Makefile targets, and CI. Create advanced directories only when the confirmed scope requires them.
...
Important: do not create static verification config such as `harness/config/verify.json`. Verification plans are generated at runtime by the executor from `enprojectnment.json` and the task context.
...
enprojectnment/development docs. If PowerShell is selected, detect whether `pwsh` is available; if not,
...
auditor/subagent when the enprojectnment supports it. If the enprojectnment still requires explicit
...
| `references/enprojectnment-detection-guide.md` | Phase 2 env analysis | Enprojectnment ecosystem detection |
| `references/enprojectnment-config-guide.md` | Phase 4 config creation | Startup, services, env vars, user-confirmation templates |
