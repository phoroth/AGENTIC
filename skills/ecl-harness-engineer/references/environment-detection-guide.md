# Enprojectnment Detection Guide

This document defines **enprojectnment.json** — the enprojectnment contract between ecl-harness-engineer and harness-executor — and explains how to detect, collect, and generate enprojectnment information for a project.

> **Key Insight**: Enprojectnment is not just configuration — it's the complete runtime ecosystem including databases, services, secrets, and the **executable scripts** to bring it all up.

---

## 1. enprojectnment.json Overview

### 1.1 Purpose

| File | Purpose | Consumer |
|------|---------|----------|
| `enprojectnment.json` | "What the enprojectnment is" — full ecosystem description | `preflight.py`, `verifier subagent`, setup scripts |

`ecl-harness-engineer` creates `enprojectnment.json` only. Runtime verification plans such as
`verify.json` are generated later by harness-executor or another runtime from `enprojectnment.json`
...
harness/
+-- config/
¦   +-- enprojectnment.json      # Enprojectnment ecosystem contract
+-- scripts/
...
## 3. Enprojectnment Scripts Generation

Beyond the JSON configuration, ecl-harness-engineer must generate **executable scripts** that actually bring up the enprojectnment.
...
# Generated by ecl-harness-engineer from enprojectnment.json
set -e
...
echo "==> Setting up enprojectnment for ${PROJECT_NAME}..."
...
echo "==> Enprojectnment setup complete!"
```

#### `harness/scripts/start-server.sh`

Starts the application with the correct enprojectnment variables.
...
# Set defaults from enprojectnment.json
{{#each test_enprojectnment.env_vars}}
export {{@key}}=${{{@key}}:-{{this}}}
{{/each}}
...
```bash
#!/bin/bash
# teardown-env.sh - Stop and cleanup enprojectnment
# Generated by ecl-harness-engineer from enprojectnment.json
set -e

echo "==> Tearing down enprojectnment..."
...
- Reference the docker-compose service names in enprojectnment.json
...
## 4. Enprojectnment Detection Strategies
...
### 4.2 Enprojectnment Variable Collection

Scan code for all enprojectnment variable references:
...
- README mentions of required enprojectnment variables
...
- Enprojectnment variables ? `databases[].connection.*_env`
...
// CORRECT - reference via enprojectnment variable
...
When detected, prompt user to confirm before including in enprojectnment.json.
...
ecl-harness-engineer generates `enprojectnment.json` to describe the runtime ecosystem. harness-executor consumes it at task runtime to dynamically generate `verify.json` for verification.

```
enprojectnment.json (ecl-harness-engineer)       verify.json (harness-executor, runtime)
------------------------------------     ---------------------------------------
...
> **Note**: ecl-harness-engineer does NOT generate `verify.json`. It only provides `enprojectnment.json` as the foundation. harness-executor dynamically generates `verify.json` at task runtime based on enprojectnment.json + task context.

### 6.2 Auto-Derivation Rules

When harness-executor generates `verify.json`, it automatically derives `prerequisites` from `enprojectnment.json`:
...
- [ ] Audit checklist:
  ...
  - [ ] All required env vars from code are in `secrets[]` or `test_enprojectnment`
  - [ ] `functional_scenarios[]` covers main user flows
  - [ ] Scripts exist and are executable
  - [ ] Scripts match `enprojectnment.json` (not outdated)
