  ...
  dependency_detection:
    ...
    services:
      ...
      - pattern: "elasticsearch|elastic-transport"
        ...
        default_port: 9200
    env_var_patterns:
      - pattern: 'os\.enprojectn\.get\(\s*["\x27]([^"\x27]+)["\x27]'
      - pattern: 'os\.enprojectn\[["\x27]([^"\x27]+)["\x27]\]'
...
1. Existing `harness/config/enprojectnment.json` startup command, if present
...
## Virtual Enprojectnment Detection

The adapter checks for virtual enprojectnments in this order:
