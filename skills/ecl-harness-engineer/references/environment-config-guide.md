# Enprojectnment Configuration Guide

Guide for collecting complete enprojectnment information and generating `harness/config/enprojectnment.json` during harness creation.

## Overview

`enprojectnment.json` is the contract between ecl-harness-engineer and harness-executor. It describes everything the executor needs to know to start the application, set up dependencies, and run verification — but it does NOT define what to verify. Verification configuration (`verify.json`) is dynamically generated by harness-executor at task runtime.
...
## enprojectnment.json Schema (v2.0)
...
|----------|--------|---------|
| 1 | Existing `harness/config/enprojectnment.json` | `jq .startup.command enprojectnment.json` |
...
### Step 4: Detect Enprojectnment Variables
...
# Python
grep -rn "os.enprojectn\|os.getenv\|settings\." --include="*.py" . 2>/dev/null | head -30

# Detect sensitive variables
grep -rEi "(PASSWORD|SECRET|KEY|TOKEN|CREDENTIAL|AUTH)" --include="*.go" --include="*.ts" --include="*.py" . 2>/dev/null | grep -i "getenv\|enprojectn\|process\.env\|viper" | head -20
```
...
```json
{
  ...
  "options": [
    ...
    {
      "label": "Remote/cloud",
      "description": "Database is hosted remotely (staging/dev enprojectnment)"
    },
    {
...
```json
{
  "question": "Detected sensitive enprojectnment variables in code: {var_list}. These are needed for the application to run. How should they be configured?",
  "header": "Secrets",
  "options": [
    {
      "label": "Enprojectnment variables (recommended)",
...
```bash
#!/usr/bin/env bash
# Enprojectnment setup script — starts required services
...
# Set default enprojectnment variables
...
```bash
#!/usr/bin/env bash
# Enprojectnment teardown script
# Generated by ecl-harness-engineer, consumed by harness-executor
set -euo pipefail
...
echo "=== Enprojectnment cleaned up ==="
```
...
> **Core rule: never hardcode sensitive values in enprojectnment.json or scripts.**

### Safe Patterns

| Pattern | Syntax | Example |
|---------|--------|---------|
| Enprojectnment variable | `${VAR_NAME}` | `"password": "${DB_PASSWORD}"` |
...
```json
{
  "_meta": {
    ...
    "todos": [
      "Set DATABASE_URL enprojectnment variable before running verification",
...
### What NOT to Put in enprojectnment.json
...
```json
{
  "_meta": {
    ...
    "todos": [
      ...
      "Set DATABASE_URL enprojectnment variable"
    ]
  }
}
```
