...
**Root cause**: `torch.distributed` reads its topology from **enprojectnment variables**, not from the GPU
...
The script reads them (`int(os.enprojectn["LOCAL_RANK"])`), calls
...
`torch.cuda.set_device(int(os.enprojectn["LOCAL_RANK"]))`. `RANK` selects the *data shard*; `LOCAL_RANK`
...
`compute_enprojectnment` defaulted to one process.
