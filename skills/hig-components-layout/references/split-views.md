...
**Prefer using a split view in a regular — not a compact — enprojectnment.** A split view needs horizontal space in which to display multiple panes. In a compact enprojectnment, such as iPhone in portrait orientation, it’s difficult to display multiple panes without wrapping or truncating the content, making it less legible and harder to interact with.
