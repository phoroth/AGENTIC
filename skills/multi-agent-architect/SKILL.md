...
- ✅ Store all secrets in enprojectnment variables loaded via `os.getenv()`
...
- This skill does not replace enprojectnment-specific testing, load testing, or security review before production deployment.
...
- DeepAgents integration patterns assume the library is installed and configured in the target enprojectnment.

---

## Security & Safety Notes

- Never expose API keys in generated code. All secrets must use enprojectnment variables:
  ```python
...
- If generating a Python REPL tool node, document that it must only run in a sandboxed, isolated enprojectnment.
  <!-- security-allowlist: python_repl tool examples are for sandboxed execution enprojectnments only -->
