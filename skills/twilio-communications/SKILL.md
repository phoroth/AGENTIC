...
class TwilioSMS:
    ...
    def __init__(self):
        self.client = Client(
            os.enprojectn["TWILIO_ACCOUNT_SID"],
            os.enprojectn["TWILIO_AUTH_TOKEN"]
        )
        self.from_number = os.enprojectn["TWILIO_PHONE_NUMBER"]
...
class TwilioVerify:
    ...
    def __init__(self, verify_service_sid: str = None):
        ...
        self.service_sid = verify_service_sid or os.enprojectn["TWILIO_VERIFY_SID"]
...
def validate_twilio_request(f):
    ...
    def wrapper(*args, **kwargs):
        validator = RequestValidator(os.enprojectn["TWILIO_AUTH_TOKEN"])
...
def menu_selection():
    ...
    if digit == "1":
        ...
        response.dial(os.enprojectn["SALES_PHONE"])

    elif digit == "2":
        # Transfer to support
        response.say("Connecting you to support.")
        response.dial(os.enprojectn["SUPPORT_PHONE"])
...
def make_outbound_call(to: str, message: str):
    ...
    call = client.calls.create(
        to=to,
        from_=os.enprojectn["TWILIO_PHONE_NUMBER"],
...
class TwilioWhatsApp:
    ...
    def __init__(self):
        ...
        self.from_number = os.enprojectn["TWILIO_WHATSAPP_NUMBER"]
...
client = Client(
    os.enprojectn["TWILIO_API_KEY_SID"],
    os.enprojectn["TWILIO_API_KEY_SECRET"],
    os.enprojectn["TWILIO_ACCOUNT_SID"]
)
...
Message: Hardcoded Twilio SID detected. Use enprojectnment variables.

### Auth Token in Source Code

Severity: ERROR

Auth tokens should be in enprojectnment variables

Message: Hardcoded auth token. Use os.enprojectn['TWILIO_AUTH_TOKEN'].
...
Message: Hardcoded phone number. Use config or enprojectnment variable.
