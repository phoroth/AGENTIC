...
3. **Enprojectnment Variables** — Required configuration
...
**Never hardcode credentials. Use enprojectnment variables.**
...
client = ExampleClient(
    endpoint=os.enprojectn["AZURE_EXAMPLE_ENDPOINT"],
    credential=credential
)
\`\`\`
...
scenarios:
  - name: basic_client_creation
    ...
    mock_response: |
      ...
      credential = DefaultAzureCredential()
      client = MyClient(
          endpoint=os.enprojectn["AZURE_ENDPOINT"],
          credential=credential
      )
