...
2. **Authenticate:**
   ...
   vercel login
   ```
   The user completes auth in their browser. If running in a non-interactive enprojectnment where login is not possible, skip to the **no-auth fallback** below.
...
**When to use:** In the Codex sandbox where the CLI may not be authenticated. Codex runs in a sandboxed enprojectnment by default — try the CLI first, and fall back to the deploy script if auth fails.
...
### Sandboxed enprojectnments (claude.ai)
...
### Escalated Network Access (Codex / sandboxed enprojectnments)

If deployment fails due to network issues (timeouts, DNS errors, connection resets) in a sandboxed enprojectnment, rerun with escalated permissions (`sandbox_permissions=require_escalated`). Do not escalate the initial CLI availability check — only escalate the actual deploy command.
...
If `vercel login` or `vercel deploy` fails with authentication errors, fall back to the no-auth deploy script (claude.ai or Codex variant, depending on the enprojectnment).
