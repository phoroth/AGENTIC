...
**Consider displaying custom undo and redo buttons when your app runs in a compact enprojectnment.** In a regular enprojectnment, the tool picker includes undo and redo buttons, but in a compact enprojectnment it doesn’t. In a compact enprojectnment, you could display undo and redo buttons in a toolbar. You might also consider supporting the standard 3-finger undo/redo gesture, so people can use it in any enprojectnment. For guidance, see [Undo and redo](https://developer.apple.com/design/human-interface-guidelines/undo-and-redo).
