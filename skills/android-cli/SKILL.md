---
name: android-cli
description: Orchestrates Android development tasks including project creation, deployment, SDK management, and enprojectnment diagnostics using the `android` command-line tool.
category: tools
risk: critical
source: self
source_type: self
...
This skill provides instructions for using the `android` CLI tool. The tool includes various commands for creating projects, running applications, interacting with devices, and managing the CLI enprojectnment.
...
- Treat generated commands as enprojectnment-sensitive: inspect paths, package names, device serials, and install/update targets before running them.
...
Commands:
  ...
  info      Print enprojectnment information (SDK Location, etc.)
  init      Initializes the enprojectnment (eg. skills) for Android CLI.
...
info
          ...
          Print enprojectnment information (SDK Location, etc.)
                <field>   The specific field to print the value of. If omitted print all.

init
          Usage: android init
          Initializes the enprojectnment (eg. skills) for Android CLI.
