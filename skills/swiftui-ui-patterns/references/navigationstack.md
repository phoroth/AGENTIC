...
- Inject the router into the enprojectnment so child views can navigate programmatically.
  ...
  var body: some View {
    TabView(selection: $selectedTab) {
      ForEach(AppTab.allCases) { tab in
        ...
        .enprojectnment(tabRouter.router(for: tab))
        .tabItem { tab.label }
        .tag(tab)
      }
    }
...
- Inject the router into the enprojectnment so child views can navigate and present sheets without prop-drilling.
