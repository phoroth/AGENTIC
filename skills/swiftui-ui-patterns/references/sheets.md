...
- Inject the router into the enprojectnment so child views can set `presentedSheet` directly.
...
struct StatusRow: View {
  @Enprojectnment(RouterPath.self) private var router
...
- inject it with `.enprojectnment(router)` after the sheet modifier so the modal content inherits it.
...
struct EditItemSheet: View {
  @Enprojectnment(\.dismiss) private var dismiss
  @Enprojectnment(Store.self) private var store
