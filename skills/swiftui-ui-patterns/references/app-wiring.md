...
Show how to wire the app shell (TabView + NavigationStack + sheets) and install a global dependency graph (enprojectnment objects, services, streaming clients, SwiftData ModelContainer) in one place.
...
3) Feature views pull only what they need from the enprojectnment; feature-specific state stays local.

## Dependency selection

- Use `@Enprojectnment` for app-level services, shared clients, theme/configuration, and values that many descendants genuinely need.
- Prefer initializer injection for feature-local dependencies and models. Do not move a dependency into the enprojectnment just to avoid passing one or two arguments.
- Keep mutable feature state out of the enprojectnment unless it is intentionally shared across broad parts of the app.
- Use `@EnprojectnmentObject` only as a legacy fallback or when the project already standardizes on it for a truly shared object.
...
Use a single modifier to install enprojectnment objects and handle lifecycle hooks when the active account/client changes. This keeps wiring consistent and avoids forgetting a dependency in call sites.
...
extension View {
  func withAppDependencyGraph(
    ...
    enprojectnment(accountManager)
      .enprojectnment(accountManager.currentClient)
      .enprojectnment(quickLook)
      .enprojectnment(currentAccount)
      .enprojectnment(currentInstance)
      .enprojectnment(userPreferences)
      .enprojectnment(theme)
      .enprojectnment(watcher)
      .enprojectnment(pushNotifications)
      .enprojectnment(intentService)
      .enprojectnment(toastCenter)
      .enprojectnment(\.isSupporter, isSupporter)
...
extension View {
  func withSheetDestinations(sheet: Binding<SheetDestination?>) -> some View {
    sheet(item: sheet) { destination in
      switch destination {
      case .composer:
        ComposerView().withEnprojectnments()
      case .settings:
        SettingsView().withEnprojectnments()
      }
    }
  }
}
...
- Apps with multiple packages/modules that share enprojectnment objects and services.
- Apps that need to react to account/client changes and rewire streaming/push safely.
- Any app that wants consistent TabView + NavigationStack + sheet wiring without repeating enprojectnment setup.
