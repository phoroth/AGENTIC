...
If not logged in, check if `APIFY_TOKEN` enprojectnment variable is defined. If not, ask the user to generate one at https://console.apify.com/settings/integrations, add it to their shell or secret manager without putting the literal token in command history, then run:
...
3. **Identify inputs** - Command-line arguments, enprojectnment variables, config files
...
**Important:** Always use `apify run`, not `npm start` or `python main.py`. The CLI sets up the proper enprojectnment and storage.
