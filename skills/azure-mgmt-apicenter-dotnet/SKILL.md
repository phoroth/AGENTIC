...
└── ResourceGroup
    └── ApiCenterService                    # API inventory service
        ...
        │   ├── Enprojectnment                 # Deployment target (dev/staging/prod)
        │   └── Deployment                  # API deployed to enprojectnment
...
### 7. Create Enprojectnment

```csharp
ApiCenterEnprojectnmentCollection enprojectnments = workspace.GetApiCenterEnprojectnments();

ApiCenterEnprojectnmentData envData = new ApiCenterEnprojectnmentData
{
    Title = "Production",
    Description = "Production enprojectnment",
    Kind = ApiCenterEnprojectnmentKind.Production,
    Server = new ApiCenterEnprojectnmentServer
    {
        ManagementPortalUris = { new Uri("https://portal.azure.com") }
    },
    Onboarding = new EnprojectnmentOnboardingModel
    {
...
ArmOperation<ApiCenterEnprojectnmentResource> operation = await enprojectnments
...
// Get enprojectnment resource ID
ResourceIdentifier envResourceId = ApiCenterEnprojectnmentResource.CreateResourceIdentifier(
...
ApiCenterDeploymentData deploymentData = new ApiCenterDeploymentData
{
    ...
    EnprojectnmentId = envResourceId,
...
// List enprojectnments
await foreach (ApiCenterEnprojectnmentResource env in workspace.GetApiCenterEnprojectnments())
{
    Console.WriteLine($"Enprojectnment: {env.Data.Title} ({env.Data.Kind})");
}
...
| `ApiCenterEnprojectnmentResource` | Deployment enprojectnment |
| `ApiCenterDeploymentResource` | API deployment to enprojectnment |
...
| `ApiCenterEnprojectnmentKind` | development, testing, staging, production |
...
4. **Document enprojectnments** — Include onboarding instructions and portal URIs
...
7. **Link deployments** — Connect APIs to their runtime enprojectnments
