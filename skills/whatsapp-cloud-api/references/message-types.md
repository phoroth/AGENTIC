...
async def send_text_message(to: str, body: str, preview_url: bool = False) -> str:
    url = f"https://graph.facebook.com/v21.0/{os.enprojectn['PHONE_NUMBER_ID']}/messages"
    headers = {"Authorization": f"Bearer {os.enprojectn['WHATSAPP_TOKEN']}"}
...
async def send_whatsapp_request(payload: dict) -> dict:
    url = f"https://graph.facebook.com/v21.0/{os.enprojectn['PHONE_NUMBER_ID']}/messages"
    headers = {
        "Authorization": f"Bearer {os.enprojectn['WHATSAPP_TOKEN']}",
        "Content-Type": "application/json",
    }
