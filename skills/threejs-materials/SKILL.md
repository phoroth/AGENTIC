...
// Or set as scene enprojectnment (affects all PBR materials)
scene.enprojectnment = envMap;

// HDR enprojectnment (recommended)
...
As of r183, `MeshLambertMaterial` and `MeshPhongMaterial` support image-based lighting (IBL) via `scene.enprojectnment`. Previously, only PBR materials (Standard/Physical) responded to enprojectnment maps set on the scene.
