...
Load credentials from enprojectnment variables (recommended):

```python
import os
service = IngestionService(mc_client=Client(session=Session(
    mcd_id=os.enprojectn["MCD_INGEST_ID"],
    mcd_token=os.enprojectn["MCD_INGEST_TOKEN"],
    scope="Ingestion",
)))
```
...
## Enprojectnment variable conventions
