...
```bash
# Find All Lambda Enprojectnment Variables
for d in */ ; do
    tail $d/scoutsuite-results/scoutsuite_results*.js -n +2 | jq '.services.awslambda.regions[].functions[] | select (.env_variables != []) | .arn, .env_variables' >> lambda-all-enprojectnment-variables.txt
done
...
## Vulnerable Training Enprojectnments
