...
COPILOTSTUDIOAGENT__ENprojectNMENTID=<enprojectnment-id>
...
# Load configuration
load_dotenv()
agents_sdk_config = load_configuration_from_env(enprojectn)
...
if __name__ == "__main__":
    run_app(APP, host="localhost", port=enprojectn.get("PORT", 3978))
```
...
CLIENT = AsyncAzureOpenAI(
    api_version=enprojectn["AZURE_OPENAI_API_VERSION"],
    azure_endpoint=enprojectn["AZURE_OPENAI_ENDPOINT"],
    api_key=enprojectn["AZURE_OPENAI_API_KEY"]
)
...
async def main():
    settings = ConnectionSettings(
        enprojectnment_id=enprojectn.get("COPILOTSTUDIOAGENT__ENprojectNMENTID"),
        agent_identifier=enprojectn.get("COPILOTSTUDIOAGENT__SCHEMANAME"),
    )
    
    token = acquire_token(
        settings,
        app_client_id=enprojectn.get("COPILOTSTUDIOAGENT__AGENTAPPID"),
        tenant_id=enprojectn.get("COPILOTSTUDIOAGENT__TENANTID"),
    )
...
3. Always use `load_configuration_from_env(enprojectn)` to load SDK configuration.
...
8. Keep secrets in enprojectnment variables, not in source code.
