...
module "vpc" {
  ...
  single_nat_gateway = var.enprojectnment != "production"
}
...
module "rds" {
  ...
  instance_class       = var.enprojectnment == "production" ? "db.t3.medium" : "db.t3.micro"
  ...
  backup_retention_period = var.enprojectnment == "production" ? 7 : 1
  deletion_protection     = var.enprojectnment == "production"
}
```
...
# Create Container Apps enprojectnment
...
### Strategy
```
1. Deploy new version to "green" enprojectnment
...
.env.staging       # Staging enprojectnment
.env.production    # Production (never commit)
```

All production secrets must be in a secrets manager, never in code or enprojectnment files.
