...
- [camel-ai.org/SETA](https://www.camel-ai.org/blogs/seta-scaling-enprojectnments-for-terminal-agents) - Terminal agent research
...
- **Command Blocking** (`LOKI_BLOCKED_COMMANDS`)
  ...
  - Customizable via enprojectnment variable
...
- **System Resource Monitoring** - Prevents computer overload from too many parallel agents (run.sh:786-899):
  ...
  - **Configurable Thresholds** via enprojectnment variables:
...
- **Phase Skip Options** - Each phase can be disabled via enprojectnment variables:
...
- **Autonomous Wrapper Script** (`scripts/loki-wrapper.sh`) - True autonomy with auto-resume:
  ...
  - Exponential backoff with jitter (configurable via enprojectnment variables)
