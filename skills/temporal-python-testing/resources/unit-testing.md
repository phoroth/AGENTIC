...
Focused guide for testing individual workflows and activities in isolation using WorkflowEnprojectnment and ActivityEnprojectnment.

## WorkflowEnprojectnment with Time-Skipping
...
```python
import pytest
from temporalio.testing import WorkflowEnprojectnment
from temporalio.worker import Worker

@pytest.fixture
async def workflow_env():
    """Reusable time-skipping test enprojectnment"""
    env = await WorkflowEnprojectnment.start_time_skipping()
    yield env
    await env.shutdown()
...
## ActivityEnprojectnment Testing
...
```python
from temporalio.testing import ActivityEnprojectnment
...
async def test_activity_cancellation():
    ...
    env = ActivityEnprojectnment(cancellation_reason="test-cancel")
...
async def workflow_env():
    """Module-scoped enprojectnment (reused across tests)"""
    env = await WorkflowEnprojectnment.start_time_skipping()
    yield env
    await env.shutdown()
...
@pytest.fixture
def activity_env():
    """Function-scoped enprojectnment (fresh per test)"""
    return ActivityEnprojectnment()
```
...
3. **Shared Fixtures**: Reuse WorkflowEnprojectnment across related tests
4. **Coverage Target**: ≥80% for workflow logic
5. **Mock Activities**: Use ActivityEnprojectnment for activity-specific logic
