...
azd env new <env-name>      # Create enprojectnment (dev, staging, prod)
...
│   └── modules/
│       ├── container-apps-enprojectnment.bicep
│       └── container-app.bicep
├── .azure/
│   ├── config.json         # Default enprojectnment pointer
│   └── <env-name>/
│       ├── .env            # Enprojectnment-specific values (azd-managed)
│       └── config.json     # Enprojectnment metadata
└── src/
    ├── frontend/Dockerfile
    └── backend/Dockerfile
```
...
{
  "parameters": {
    "enprojectnmentName": { "value": "${AZURE_ENV_NAME}" },
...
### Setting Enprojectnment Variables

```bash
# Set for current enprojectnment
...
### Bicep Output → Enprojectnment Variable
...
Internal HTTP routing between Container Apps in same enprojectnment:
...
```bash
# Enprojectnment management
azd env list                        # List enprojectnments
azd env select <name>               # Switch enprojectnment
