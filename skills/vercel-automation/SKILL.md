...
- `target`: Filter by enprojectnment ('production', 'preview')
...
### 3. Manage Enprojectnment Variables

**When to use**: User wants to add, list, or remove enprojectnment variables for a project

**Tool sequence**:
1. `VERCEL_LIST_PROJECTS` - Find the project ID [Prerequisite]
2. `VERCEL_LIST_ENV_VARIABLES` - List existing env vars [Required]
3. `VERCEL_ADD_ENprojectNMENT_VARIABLE` - Add a new env var [Optional]
4. `VERCEL_DELETE_ENprojectNMENT_VARIABLE` - Remove an env var [Optional]

**Key parameters**:
- `projectId`: Target project identifier
- `key`: Enprojectnment variable name
- `value`: Enprojectnment variable value
- `target`: Array of enprojectnments ('production', 'preview', 'development')
- `type`: Variable type ('plain', 'secret', 'encrypted', 'sensitive')

**Pitfalls**:
- Enprojectnment variable names must be unique per target enprojectnment
...
- Env vars are scoped to enprojectnments (production, preview, development)
...
| Add env var | VERCEL_ADD_ENprojectNMENT_VARIABLE | projectId, key, value, target |
| Delete env var | VERCEL_DELETE_ENprojectNMENT_VARIABLE | projectId, id |
