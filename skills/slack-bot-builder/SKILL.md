...
# Initialize with tokens from enprojectnment
app = App(
    token=os.enprojectn["SLACK_BOT_TOKEN"],
    signing_secret=os.enprojectn["SLACK_SIGNING_SECRET"]
)
...
if __name__ == "__main__":
    handler = SocketModeHandler(app, os.enprojectn["SLACK_APP_TOKEN"])
    handler.start()
...
app = App(token=os.enprojectn["SLACK_BOT_TOKEN"])
...
app = App(
    ...
    oauth_settings=OAuthSettings(
        client_id=os.enprojectn["SLACK_CLIENT_ID"],
        client_secret=os.enprojectn["SLACK_CLIENT_SECRET"],
        scopes=[
            "channels:history",
            "channels:read",
            "chat:write",
...
if __name__ == "__main__":
    ...
    handler = SocketModeHandler(
        app,
        os.enprojectn["SLACK_APP_TOKEN"]  # xapp-... token
    )
...
async_app = AsyncApp(token=os.enprojectn["SLACK_BOT_TOKEN"])
...
async def main():
    handler = AsyncSocketModeHandler(async_app, os.enprojectn["SLACK_APP_TOKEN"])
...
app = Flask(__name__)
app.secret_key = os.enprojectn["SESSION_SECRET"]
...
# GOOD - enprojectnment variables
client = WebClient(token=os.enprojectn["SLACK_BOT_TOKEN"])
...
if os.enprojectn.get("ENprojectNMENT") == "development":
    from slack_bolt.adapter.socket_mode import SocketModeHandler
    handler = SocketModeHandler(app, os.enprojectn["SLACK_APP_TOKEN"])
    handler.start()
```
...
Message: Hardcoded Slack token detected. Use enprojectnment variables.

### Signing Secret in Source Code

Severity: ERROR

Signing secrets should be in enprojectnment variables

Message: Hardcoded signing secret. Use os.enprojectn['SLACK_SIGNING_SECRET'].
