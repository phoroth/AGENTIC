...
- Prefer `@State`, `@Enprojectnment`, `@Query`, `.task`, `.task(id:)`, and `onChange` before reaching for a view model.
...
- wrap values already available through `@Enprojectnment`,
...
- multiple views share the same presentation-specific state and that state is not better modeled as app-level enprojectnment data.

Even then, keep the view model small, explicit, and non-optional when possible.

## Preferred pattern: local state plus enprojectnment

```swift
struct FeedView: View {
    @Enprojectnment(BlueSkyClient.self) private var client
...
- dependencies come from the enprojectnment instead of a wrapper object,
...
struct BookListView: View {
    ...
    @Enprojectnment(\.modelContext) private var modelContext
...
- Remove view models that only wrap enprojectnment dependencies or local view state.
...
- `@Enprojectnment` for shared dependencies,
