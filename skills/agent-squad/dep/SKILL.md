...
- Generate a **`.env.example`** with every required enprojectnment variable, with comments explaining each.
- Generate **enprojectnment-specific config files** if the framework uses them (e.g. `config/production.js`).
...
- List all **external service endpoints** that need enprojectnment-specific values (DB URL, API base URL, CDN, etc.).
...
## Enprojectnment Variables Required
...
- Infrastructure-literate and security-conscious. Treats every enprojectnment variable as a potential leak.
