...
**Multi-Enprojectnment CI/CD Pipeline**
```yaml
...
jobs:
  ...
  build:
    ...
    strategy:
      matrix:
        enprojectnment: [development, staging, production]
    steps:
      - uses: actions/checkout@v4

      - name: Set up build enprojectnment
        uses: actions/setup-node@v4
        with:
          node-version: ${{ env.NODE_VERSION }}
          cache: 'npm'
      ...
      - name: Build application
        run: npm run build
        env:
          NODE_ENV: ${{ matrix.enprojectnment }}
      ...
      - name: Build Docker image
        run: |
          docker build \
            ...
            -t ${{ github.repository }}:${{ matrix.enprojectnment }}-${{ github.sha }} \
            -t ${{ github.repository }}:${{ matrix.enprojectnment }}-latest \
            .

      - name: Scan Docker image
        uses: aquasecurity/trivy-action@master
        with:
          image-ref: ${{ github.repository }}:${{ matrix.enprojectnment }}-${{ github.sha }}
          format: 'sarif'
      ...
      - name: Push to registry
        ...
        run: |
          ...
          docker push ${{ github.repository }}:${{ matrix.enprojectnment }}-${{ github.sha }}
          docker push ${{ github.repository }}:${{ matrix.enprojectnment }}-latest

      - name: Upload artifacts
        uses: actions/upload-artifact@v3
        with:
          name: build-${{ matrix.enprojectnment }}
          path: |
            dist/
            build/
            .next/
  ...
  # Deploy
  deploy:
    name: Deploy to ${{ matrix.enprojectnment }}
    needs: build
    runs-on: ubuntu-latest
    if: github.event_name != 'pull_request'
    strategy:
      matrix:
        enprojectnment: [staging, production]
        exclude:
          - enprojectnment: production
            branches: [develop]
    enprojectnment:
      name: ${{ matrix.enprojectnment }}
      ...
      - name: Deploy to ECS
        ...
        run: |
          ...
          aws ecs register-task-definition \
            --family myapp-${{ matrix.enprojectnment }} \
            --container-definitions "[{
              \"name\": \"app\",
              \"image\": \"${{ github.repository }}:${{ matrix.enprojectnment }}-${{ github.sha }}\",
              \"enprojectnment\": [{
                \"name\": \"ENprojectNMENT\",
                \"value\": \"${{ matrix.enprojectnment }}\"
              }]
            }]"
          
          # Update service
          aws ecs update-service \
            --cluster ${{ matrix.enprojectnment }}-cluster \
            --service myapp-service \
            --task-definition myapp-${{ matrix.enprojectnment }}
          
          # Get service URL
          echo "url=https://${{ matrix.enprojectnment }}.example.com" >> $GITHUB_OUTPUT

      - name: Notify deployment
        uses: 8398a7/action-slack@v3
        with:
          status: ${{ job.status }}
          text: Deployment to ${{ matrix.enprojectnment }} ${{ job.status }}
  ...
  verify:
    ...
    steps:
      ...
      - name: Run smoke tests
        run: |
          npm run test:smoke -- --url https://${{ matrix.enprojectnment }}.example.com

      - name: Run E2E tests
        uses: cypress-io/github-action@v5
        with:
          config: baseUrl=https://${{ matrix.enprojectnment }}.example.com
          record: true
        env:
      ...
      - name: Performance test
        run: |
          ...
          sitespeed.io https://${{ matrix.enprojectnment }}.example.com \
      ...
      - name: Security scan
        run: |
          ...
          zaproxy/action-baseline -t https://${{ matrix.enprojectnment }}.example.com
```
...
**Development Enprojectnment Setup**
```bash
#!/bin/bash
# scripts/setup-dev-enprojectnment.sh
...
# Setup enprojectnment variables
setup_enprojectnment() {
    echo "Setting up enprojectnment variables..."
...
main() {
    ...
    echo "✅ Development enprojectnment setup complete!"
    echo ""
    echo "Next steps:"
